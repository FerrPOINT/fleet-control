...
test('workflow bindings remain usable on the live fleet across themes and widths', async ({
  page,
}) => {
  test.setTimeout(180_000)
  await page.goto('http://localhost:7742/workflows')
  await page.getByLabel('Email').fill(account.email)
  await page.getByLabel('Пароль').fill(account.password)
  await page.getByRole('button', { name: 'Войти', exact: true }).click()
  await expect(page.getByRole('heading', { name: 'Привязки процессов' })).toBeVisible()
  ...
  await expect(page.getByLabel('Поиск по агенту и процессу')).toBeVisible()
  ...
  await page.getByLabel('Статус').selectOption('stale')
  await expect(page.getByRole('button', { name: 'Перепривязать' }).first()).toBeEnabled()
  await expect(page.getByText('Загрузка привязок...')).toHaveCount(0)
  ...
  for (const theme of ['light', 'gray', 'dark']) {
    ...
    for (const [width, height] of [
      ...
      const screenshot = fileURLToPath(
        new URL(
          evidence
            ? `../../docs/assets/screens/2026-09-19-workflows/${theme}-${width}.png`
            : `../../../.local/screenshots/fleet-workflows-${theme}-${width}.png`,
          import.meta.url,
        ),
      )
